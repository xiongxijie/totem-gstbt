...
struct _TotemPreferencesDialog {
	...
	GtkCheckButton *tpw_disable_kbd_shortcuts;
...
totem_preferences_dialog_constructed (GObject *object)
{
	...
	/* Sound output type, thru bvw's property "audio-output-type" */
	...
	g_settings_bind (totem->settings, "disable-keyboard-shortcuts",
			 prefs->tpw_disable_kbd_shortcuts, "active", G_SETTINGS_BIND_DEFAULT);
	totem->disable_kbd_shortcuts = g_settings_get_boolean (totem->settings, "disable-keyboard-shortcuts");
...
totem_preferences_dialog_class_init (TotemPreferencesDialogClass *klass)
{
	...
	gtk_widget_class_bind_template_child (widget_class, TotemPreferencesDialog, tpw_disable_kbd_shortcuts);
